...
使用`||`表示逻辑**或**，当且仅当其一个或多个值为真时，其运算结果为真；使用`&&`表示逻辑**与**，当且仅当所有值为真时，其运算结果才为真。

Javascript 的逻辑运算符除了常规的用法，还能一些额外的附加特性。
...
`||`运算符首先会从左到右依次计算值的个数，在处理每一个值时，都将其转化为 Boolean。如果结果是`true`，就停止计算并返回这个值的初始值；如果所有的值都被计算过了(也就是所有的值的转换结果都是 `false`)，则返回最后一个值的初始值。

换句话说，一个或运算`||`的链，会返回第一个真值，如果不存在真值，则返回该链的最后一个值。例如：

```javascript
( null || 1 )  // 返回 1
( undefined || null || 0 )  // 返回 0(都是假值，返回最后一个值)
```

利用这个特性，可以获取变量列表或者表达式中的第一个真值：
...
( firstName || lastName || nickName || "Anonymous" );  // 最终返回 SuperCoder
```

`||`的另一个用途是所谓的**短路求值**。这指的是`||`对其参数进行处理，直到达到第一个真值，会立即返回该值而无需处理其他参数。这里的参数可以变量、表达式、函数调用等。
...
`&&`运算符会先从左到右计算值的个数，在处理每一个值时，都将其转化为 Boolean。如果结果是`false`，就停止计算并返回这个值的初始值。

换句话说，一个与运算`&&`的链，会返回第一个假值，如果没有假值就返回最后一个值。
...
同样`&&`也可以用作短路求值，`&&`会对其参数进行处理，直到达到第一个假值，会立即返回该值。
