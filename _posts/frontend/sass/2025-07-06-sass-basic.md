...
Sass 字符串(Strings)支持**有引号字符串**(`'`或者`"`定义)和**无引号字符串**，在编译 CSS 文件时不会改变其类型。可以使用`string.unquote($string)`将有引号字符串转换成无引号字符串，使用`string.quote($string)`将无引号字符串转换成无引号字符串。

> 在使用插值语句`#{}`时，有引号字符串将被编译为无引号字符串。
{: .prompt-tip}

一般来说，最好使用有引号字符串，除非需要特别使用 CSS 属性值，而这个属性使用了不带引号的字符串。

Sass 中有许多[字符串函数](https://sass-lang.com/documentation/modules/string)，索引 1 表示字符串的第一个字符，索引 -1 表示字符串中的最后一个字符。
...
Sass 对基于实际单位计算工作方式的单位操作提供了强大的支持。当两个数值相乘时，它们的单位也做相对应的计算，例如`4px*6px: 24px*px`，`div(5px, 2s): 2.5px/s`。一个数字的分子、分母可以是任意数量的单位。

Sass 将在兼容的单位之间自动转换，如果尝试组合不兼容的单位时，如`1in+1em`，Sass 则会抛出错误。

应该特别避免使用像`#{$number}px`这样的插值语句。这实际上并没有创建一个数值。它只是创建一个无引号字符串，不能与任何数值或者函数进行运算。尽量让数值单位简洁，例如`$number*1px`或者直接给`$number`赋值一个单位。

> 不应该在数值中使用插值语句！
{: .prompt-danger}

Sass 的百分比和其他单位一样，但是它们不能与小数互换，因为在 CSS 中小数和百分比意味着不同的东西。例如，`50%`是一个以`%`为单位的数值。可以使用`div($percentage, 100%)`将`$percentage`转换成对应的小数，使用`$decimal*100%`或者`percentage($decimal)`计算对应小数的百分比。
