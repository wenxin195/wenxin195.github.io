...
Sass(Syntactically Awesome Style Sheets)是一种功能强大的 CSS 预处理器，它在标准 CSS 的基础上进行了扩展，使开发者能够以更加结构化、易于维护且高效的方式编写样式代码。在完全兼容 CSS 的前提下，Sass 能够有效减少代码冗余，提高样式的可复用性，并简化整体的开发与设计流程。

<!--more-->

Sass 引入了许多超越传统 CSS 的高级特性，包括[变量](https://sass-lang.com/documentation/variables)、[样式规则](https://sass-lang.com/documentation/style-rules#nesting)、[Mixins 宏命令](https://sass-lang.com/documentation/at-rules/mixin)、[继承](https://sass-lang.com/documentation/at-rules/extend/)以及[函数](https://sass-lang.com/documentation/modules)等。这些功能不仅让样式管理更加灵活，也显著提升了代码的可维护性和开发效率，使开发者能够更轻松地组织、复用和管理样式表。

## Sass 的基本结构

Sass 样式表由 **语句** (Statements)和 **表达式** (Expressions)组成。语句是定义 **样式表结构** 和 **行为** 的指令，分为以下几类：

- 通用语句：这些语句可以在 Sass 样式表的 **任何位置** 使用。
  - 变量声明，例如`$primary-color: #c6538c`；
  - 控制指令，例如`@if`、`@each`、`@for`和`@while`等；
  - 调试和错误处理指令，例如`@error`、`@warn`和`@debug`等。
- CSS 语句：这些语句生成 CSS 样式，可在除`@function`之外的任何位置使用。
  - 样式规则(Style rules)，例如`h1 { color: $primary-color; }`；
  - CSS 中的 at-Rules，例如`@media`、`@font-face`或`@keyframes`；
  - 使用了`@include`的 Mixins 宏命令；
  - 用于控制输出嵌套的`@at-root`规则；
- 顶部语句：这些语句仅限于在 **样式表的顶部** 或 **嵌套在顶部的 CSS 语句** 中使用。
  - 使用了`@use`的加载模块，以及使用了`@import`的导入模块；
  - 使用了`@mixin`的 Mixins 宏命令，以及使用了`@function`的函数定义。
- 其他语句：
  - 属性声明，例如：`font-size: 16px;`，属性声明仅可在 **样式规则** 和某些 **CSS 中的 at-Rules** 中使用；
  - 使用了`@extend`的继承语句，仅可在 **样式规则** 中使用。

表达式出现在 **属性** 或 **变量声明** 的右侧，并计算它们的值。Sass 表达式(将其统称为 *SassScript*)比标准 CSS 值强大得多。它们包括：
...
- 文字表达式(Literal)：用来表示 **静态值**。
  ...
  - 布尔表达式(Boolean)以及 Nulls 值；
- 运算符表达式：支持数学计算、比较和逻辑运算等，详见 [Sass Operators](https://sass-lang.com/documentation/operators/)。
- 其他表达式：包括变量、函数调用和父级选择器`&`等。

通过使用这些强大的特性，Sass 让开发者能够编写更清晰、更模块化、可扩展的样式表，从而改变 CSS 的编写和维护方式。

## Sass 变量

Sass 变量使用`<variable>: <expression>`的方式来定义变量，其中变量名以`$`符号作为前缀。例如：
...
h1 {
  $color: green; // 局部变量，这会 Shadowing 原来的全局变量
  color: $color;
}

p {
  color: $color; // 这会使用全局变量
}
```
...
h1 {
  $global-color: green !global;  // 这修改了全局变量的赋值
  color: $global-color;
}
...
Sass 变量可以引用其他变量，从而实现模块化和可复用的样式。这对于创建值之间的关系(例如颜色和边框)十分便利，例如：
...
Sass 允许使用`!default`为变量分配默认值，仅当 **变量未定义** 或为 **空值** 时，Sass 才会为其才会赋值。这在库文件中尤其有用，可实现下游文件的变量自定义而不覆盖预定义值：
...
// style.scss
@use 'library';
$content: "Custom content" !default; // 这会被忽略，因为 `$content` 已经被定义过了
...
还可以使用`@use <url> with ( <variable>: <value>[, ...] )`在库文件中配置变量，前提是 **变量是在样式表的顶部，且使用`!default`声明的**：
...
在这个例子中，`$black`由`#000`被修改为了`#222`，`$border-radius`由`0.25rem`被修改为了`0.1rem`，`$box-shadow`使用更新后的`$black`值。

### 控制流中的变量

控制流中的变量(例如`@if`、`@for`等)具有独特的作用域规则。赋值操作不会创建新局部变量，而是 **更新** 现有变量(通常是全局变量)。这允许在循环或条件语句中进行动态更新：
...
@if $dark-theme {
  $primary-color: color.adjust($primary-color, $lightness: -60%);
  $accent-color: color.adjust($accent-color, $lightness: 60%);
}
...
```css
p {
  font: 12px/30px;  /* 表示字体大小为 12px，行高为 30px */
}
```
...
$transition-speed: math.div(1s, 50px); // 即 0.02s/px
...
.slider {
  @include move(10px, 120px); // 时长为：(120px - 10px) * 0.02s/px = 2.2s
}
```
...
在定义 Mixins 宏命令或者带有可变参数`$args...`时，会生成一个名为 **参数列表** 的特殊列表。该列表包含所有传递的参数，并确需通过`meta.keywords($args)`以映射的形式访问关键字参数。
...
.adjusted {
  background: color.adjust($base-color, $lightness: +10%); // 调亮颜色值
}
```
...
样式规则([Style rules](https://sass-lang.com/documentation/style-rules))是 Sass 的核心组成部分。属性声明(Property declarations)用于定义应用于匹配选择器的元素的样式。Sass 通过简化书写方式和提供自动化功能，使样式的编写更加高效。声明的值可以是任意有效的 *SassScript* 表达式，这些表达式会在编译时求值，并被包含在生成的 CSS 中。
...
嵌套([Nesting](https://sass-lang.com/documentation/style-rules/#nesting))是 Sass 提供的一种特性，它允许开发者按照层级关系组织相关样式，从而减少重复书写父选择器，同时提升代码的可读性。这对于管理复杂的 CSS 结构尤其有帮助，使样式层次更加清晰。

在 Sass 中 CSS 规则可以相互嵌套。当一个规则包含多个选择器时，会生成 **复合选择器**，其中外层选择器自动成为内层选择器的父级，从而简化了层级关系的表达。例如：
...
如果`&`在样式规则之外使用，则将返回`null`。这可以在 Mixins 宏命令中加以利用，实现条件化样式应用：
...
CSS 自定义属性(即 CSS 变量)几乎允许任何文本作为其值，这使得它们可以被 JavaScript 访问，并且具有高度的动态性。Sass 处理自定义属性声明的方式有所不同，它会将所有标记原样传递给 CSS；然而插值语句`#{}`除外，这是注入动态值的唯一方法：
...
Mixins 是一项强大的功能，它允许定义可复用的 CSS 样式块，从而避免使用例如`.float-left`这类非语义化的类名。借助 Mixins 宏命令，开发者可以封装 CSS 规则、Sass 特性，甚至动态变量，从而创建灵活、可重复使用的样式。

当网站存在小规模重复的样式模式(如统一的颜色或字体)时，Sass 变量是保持一致性的理想选择。然而随着样式变得越来越复杂，重复使用大块 CSS 代码会变得非常繁琐，这时 Mixins 就能够高效地重复使用整个样式块。

### @mixin

`@mixin`指令用于定义可复用的样式块。例如：
...
定义好 Mixins 宏命令之后，需要通过`@include`进行引用，例如：
...
p {
  @include sexy-border(blue);  // 使用了默认参数值
}

h1 {
  @include sexy-border(blue, 2in);  // 没有使用默认参数值
}
```
...
Mixins 可以接受内容块(Content Blocks)，使用`@content`的特定位置注入自定义样式，这对于有条件地应用样式非常有用。例如针对特定浏览器：
...
内容块在其定义的作用域内进行求值，而不是在 Mixins 的作用域内。这意味着 Mixins 内部的局部变量无法被内容块访问，变量会被解析为其全局或周围作用域的值：
...
.colors {
  @include colors {
    color: $color;  // 这会解析为全局变量 $color(即白色)，而非 Mixin 中的 $color(即蓝色)。
  }
}
```

类似地，在周围范围内定义的变量可以在内容块内使用：
...
`@use`可以从另一个 Sass 样式表(称为模块，Modules)加载 Mixins、函数、变量或者其他成员。与旧版`@import`不同，使用`@use`加载的模块无论被引用多少次，在编译后的 CSS 输出中都只会被包含一次，从而确保代码更简洁、更高效。
...
在此示例中，加载了`corners`模块，并且其成员(Mixins 宏命令`rounded`和`$radius`变量)使用`corners`命名空间进行访问。

可以使用`as`关键字为模块分配自定义命名空间(Namespace)，从而使代码更简洁或更具上下文相关性：
...
.button {
  @include corners.rounded;
  padding: 5px + corners.$_radius; // 这会报错，因为 $_radius 是私有成员，在下游文件内不可访问
}
```
...
这时`library.$color`在最终输出中更新为`blue`。
...
为了使成员名称的命名更具语义化，可以使用`<prefix>-*`为所有转发的成员添加前缀：
...
在这个示例中，Mixins 宏命令`reset`被分发成为了`list-reset`，提高了下游样式表的可读性。
...
在这个例子中，`list-reset`和`$horizo​​ntal-list-gap`被隐藏，使得下游样式表无法使用它们，但`list-horizo​​ntal`仍然可以访问。
...
Sass 虽然仍能支持`@import`，但在最新版中已被弃用，取而代之的是 `@use`和`@forward`。`@import`将导入的 SCSS 或 Sass 文件合并为一个 CSS 输出，使变量和混合宏在全局范围内可用。然而它有几个缺点：
...
在设计网页的时候常常遇到一个元素使用的样式与另一个元素完全相同，但又添加了额外的样式。通常会在 HTML 中给元素定义两个类：一个通用样式和一个特殊样式。假设现在要设计一个 **普通错误样式** 与一个 **严重错误样式**，我们一般会这样写：
...
Sass 会智能地组合选择器以避免冗余。它不会生成像`.error .seriousError`这样不必要的重复代码，并且会移除无法匹配任何元素的选择器(例如`#main #footer`)。
...
单个选择器可以继承多个选择器的样式，并组合它们的属性。考虑以下示例：
...
此时`.criticalError`继承自`.seriousError`的样式，而`.seriousError`又继承自`.error`，从而形成清晰的样式层级结构。上面的代码编译后得到：
...
例如想编写一个与外部选择器和元素选择器匹配的选择器，那么则可以编写一个类似下面的 Mixins，在这个示例中使用了`selector.unify()`函数将`&`与下游样式表的选择器组合起来：
...
Sass 提供了类似编程语言的控制流语句，根据条件生成样式或通过循环减少重复代码，这使得样式可以根据条件或数据动态生成，而不再是静态声明。

### 判断语句

`@if`可以根据布尔表达式的结果有条件地执行代码块，使用`@else if`和`@else`处理多种分支：
...
Sass 提供了`@for`和`@while`语句来实现循环逻辑。`@for`的语法为：

- `@for <variable> from <expression> to <expression> { ... }`，这种写法不包含结束值(包前不包后)
- `@for <variable> from <expression> through <expression> { ... }`，这种写法包含结束值(全闭区间)
...
`@while`语句只要满足特定条件就会持续循环，适合处理更复杂的逻辑：
...
`@each`还支持多变量解构赋值，非常适合处理图标、颜色主题、断点等场景，例如:
...
函数(`@function`)旨在让开发者定义一段可复用的计算逻辑或者返回一个值，常用于颜色运算、尺寸计算、单位转换等场景。与 Mixins 宏命令不同，函数不直接输出样式规则，而是作为属性值的一部分使用。

要定义一段函数，需要使用`@function`定义函数，并必须通过`@return`返回结果：
...
2. [https://sass.hk/docs/](https://sass.hk/docs/)
3. [https://www.w3schools.com/sass/default.asp](https://www.w3schools.com/sass/default.asp)
